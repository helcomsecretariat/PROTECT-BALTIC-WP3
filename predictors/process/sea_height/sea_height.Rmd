...
# Sea surface height above sea level coefficient of variation
```{r}
...
temp <- temp*grid
if(exnon){
  writeRaster(temp, "outputs/sea_surface_height_cv_250m.tif", overwrite = TRUE, # Use to export non-resampled layer
              datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
}
...
writeRaster(temp_res, "outputs/sea_surface_height_cv_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
