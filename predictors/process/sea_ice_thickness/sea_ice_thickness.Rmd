...
temp <- temp*grid
if(exnon){
  writeRaster(temp, "outputs/sea_ice_thickness_sd_250m.tif", overwrite = TRUE,
              datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
}
...
temp <- app(rst, fun = "sd", na.rm = TRUE)
temp_res <- resint(temp, grid, nit = 50, win_size = 3, method = "cubic")
writeRaster(temp_res, "outputs/sea_ice_thickness_sd_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
