...
temp <- temp*grid
#writeRaster(temp, "outputs/secchi_depth_mean_250m.tif", overwrite = TRUE, # Use to export non-resampled layer
...
writeRaster(temp_res, "outputs/secchi_depth_mean_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```

# Secchi depth standard deviation
```{r}
temp <- app(rst, fun = "sd")
temp <- project(temp, grid, method = "near")
temp <- temp*grid
#writeRaster(temp, "outputs/secchi_depth_sd_250m.tif", overwrite = TRUE, # Use to export non-resampled layer
#            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))

# Resample to higher resolution
temp <- app(rst, fun = "sd")
temp_res <- resint(temp, grid, nit = 50, win_size = 3, method = "cubic")
writeRaster(temp_res, "outputs/secchi_depth_sd_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
