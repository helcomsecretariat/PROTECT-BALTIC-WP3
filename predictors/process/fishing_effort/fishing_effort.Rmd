...
for(i in 2012:2020){
  ...
  temp <- temp * grid
  names(temp) <- paste0(i)
  writeRaster(temp, paste0("outputs/yearly_with_error/fishing_effort_", i, "_250m.tif"))
...
for(i in 2012:2020){
  ...
  temp <- temp * grid
  names(temp) <- paste0(i)
  writeRaster(temp, paste0("outputs/yearly_error_removed/fishing_effort_", i, "_250m.tif"))
  if(i == 2012){gfw <- temp} else {
    gfw <- c(gfw, temp)
  }
}
...
## With error 
```{r}
gfw_mean <- app(gfw_err, fun = mean, na.rm = TRUE)
gfw_mean[is.na(gfw_mean)] <- 0
gfw_mean[is.na(grid)] <- NA

writeRaster(gfw_mean, "outputs/fishing_effort_mean_with_error_250m.tif", overwrite = TRUE,
...
writeRaster(gfw_mean, "outputs/fishing_effort_mean_without_error_250m.tif", overwrite = TRUE,
            datatype = "INT2S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
plot(gfw_mean)
```
...
writeRaster(gfw_q, "outputs/fishing_effort_quantiles_250m.tif", overwrite = TRUE,
            datatype = "INT2S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
