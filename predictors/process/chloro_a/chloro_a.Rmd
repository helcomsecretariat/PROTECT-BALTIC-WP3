...
# Chlorophyll-a at the surface coefficient of variation
```{r}
...
temp <- temp*grid
if(exnon){
  writeRaster(temp, "outputs/chloro_a_cv_250m.tif", overwrite = TRUE, # Use to export non-resampled layer
              datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
}
...
writeRaster(temp_res, "outputs/chloro_a_cv_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
