...
temp <- temp*grid
if(exnon){
  writeRaster(temp, "outputs/sea_ice_fraction_mean_250m.tif", overwrite = TRUE, # Use to export non-resampled layer 
...
temp_res[temp_res < 0] <- 0 # Fix bad cubic interpolations giving slight negative
writeRaster(temp_res, "outputs/sea_ice_fraction_mean_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```

# Sea ice fraction coefficient of variation
```{r}
...
temp <- temp*grid
if(exnon){
  writeRaster(temp, "outputs/sea_ice_fraction_cv_250m.tif", overwrite = TRUE, # Use to export non-resampled layer
              datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
}
...
writeRaster(temp_res, "outputs/sea_ice_fraction_cv_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
