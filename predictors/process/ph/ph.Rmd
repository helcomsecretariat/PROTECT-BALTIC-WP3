...
for(i in 1:length(l)){
  if(i == 1){r <- rast(l[i])[["ph_depth=0.50164622"]]} else {
    r <- c(r, rast(l[i])[["ph_depth=0.50164622"]])
  }
}
...
temp <- temp*grid
writeRaster(temp, "outputs/ph_mean_250m.tif", overwrite = TRUE, 
...
writeRaster(temp_res, "outputs/ph_mean_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
...
temp <- project(temp, grid, method = "near")
temp <- temp*grid
writeRaster(temp, "outputs/ph_sd_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))

## Resample to higher resolution
temp <- app(r, fun = sd)
temp_res <- resint(temp, grid, nit = 50, win_size = 3, method = "cubic")
writeRaster(temp_res, "outputs/ph_sd_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
