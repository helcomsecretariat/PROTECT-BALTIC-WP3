...
temp <- temp*grid
writeRaster(temp, "outputs/nppv_mean_250m.tif", overwrite = TRUE, 
...
writeRaster(temp_res, "outputs/nppv_mean_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
...
temp <- project(temp, grid, method = "near")
temp <- temp*grid
writeRaster(temp, "outputs/nppv_sd_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))

## Resample to higher resolution
temp <- app(r, fun = sd)
temp_res <- resint(temp, grid, nit = 30, win_size = 3, method = "cubic")
writeRaster(temp_res, "outputs/nppv_sd_resample_250m.tif", overwrite = TRUE, 
            datatype = "FLT4S", gdal=c("COMPRESS=ZSTD", "PREDICTOR=2"))
```
